...
import 'package:mafuriko/core/routes/refresh.dart';
...
import 'package:mafuriko/features/onboarding/screens/splash_screen.dart';
...
final GoRouter router = GoRouter(
  refreshListenable: GoRouterRefreshBloc(sl<AuthBloc>()),
  redirect: (context, state) {
    final authState = context.read<AuthBloc>().state;
    log(' authentication state is $authState');
    log(' path state is ${state.uri.path}');
    if (authState is AuthSuccess && state.uri.path == Paths.initialPath) {
      return '/${Paths.home}'; // Rediriger vers home si authentifié
    }
    if (authState is AuthUnauthenticated &&
        state.uri.path == Paths.initialPath) {
      return '/${Paths.onboarding}'; // Rediriger vers Onboarding si non authentifié
    }
    return null;
  },
  routes: <RouteBase>[
